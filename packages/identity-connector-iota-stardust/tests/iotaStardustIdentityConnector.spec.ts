...
import { IotaStardustIdentityConnector } from "../src/iotaStardustIdentityConnector";
import { IotaStardustIdentityResolverConnector } from "../src/iotaStardustIdentityResolverConnector";
import { IotaStardustIdentityUtils } from "../src/iotaStardustIdentityUtils";
import type { IIotaStardustIdentityConnectorConfig } from "../src/models/IIotaStardustIdentityConnectorConfig";
...
describe("IotaStardustIdentityConnector", () => {
		...
		expect(
			() =>
				new IotaStardustIdentityConnector(
					undefined as unknown as { config: IIotaStardustIdentityConnectorConfig }
				)
		).toThrow(
	...
	test("can fail to construct with no config", () => {
		expect(
			() =>
				new IotaStardustIdentityConnector(
					{} as unknown as { config: IIotaStardustIdentityConnectorConfig }
				)
		).toThrow(
			expect.objectContaining({
				name: "GuardError",
	...
	test("can fail to construct with no config.clientOptions", () => {
		expect(
			() =>
				new IotaStardustIdentityConnector({ config: {} } as unknown as {
	...
	test("can create a document", async () => {
		...
		console.debug(
			"DID Document",
			`${process.env.TEST_EXPLORER_URL}addr/${IotaStardustIdentityUtils.didToAddress(testDocument.id)}?tab=DID`
		);
	});

	test("can fail to resolve a document with no id", async () => {
		const identityResolverConnector = new IotaStardustIdentityResolverConnector({
			config: {
				clientOptions: TEST_CLIENT_OPTIONS,
				vaultMnemonicId: TEST_MNEMONIC_NAME
			}
