...
import { TEST_WALLET_KEY_PAIR } from "./testWallet";
import { MemoryIdentityProvider } from "../src/MemoryIdentityProvider";

let testDocument: IDidDocument;
let testDocumentId: string;
let testDocumentAssertionId: string;
let holderDocument: IDidDocument;
...
describe("MemoryIdentityProvider", () => {
	...
	test("can create a document", async () => {
		...
		testDocument = await identityProvider.createDocument(
			TEST_WALLET_KEY_PAIR.privateKey,
			TEST_WALLET_KEY_PAIR.publicKey
		);
		testDocumentId = testDocument.id;
		expect(testDocument.id.slice(0, 10)).toBe("did:mem:0x");
	...
	test("can resolve a document id", async () => {
		const identityProvider = new MemoryIdentityProvider({
			initialValues: { [testDocumentId]: testDocument }
		});

		const doc = await identityProvider.resolveDocument(testDocumentId);
		expect(doc.id.slice(0, 10)).toBe("did:mem:0x");
	...
	test("can create a verifiable credential", async () => {
		...
		holderDocument = await identityProvider.createDocument(
			TEST_WALLET_KEY_PAIR.privateKey,
			TEST_WALLET_KEY_PAIR.publicKey
		);
		...
		const assertionMethod = (
			holderDocument.assertionMethod as IDidDocumentVerificationMethod[]
		)?.find(v => v.publicKeyJwk?.x === Converter.bytesToBase64Url(TEST_WALLET_KEY_PAIR.publicKey));
		...
		expect(subject.id.startsWith("did:mem")).toBeTruthy();
		expect(subject.degreeName).toEqual("Bachelor of Science and Arts");
		expect(subject.name).toEqual("Alice");
		expect(result.verifiableCredential.issuer.startsWith("did:mem")).toBeTruthy();
		expect(result.verifiableCredential.issuanceDate).toBeDefined();
		expect(result.verifiableCredential.credentialStatus?.id?.startsWith("did:mem")).toBeTruthy();
	...
	test("can create a verifiable presentation", async () => {
		const identityProvider = new MemoryIdentityProvider({
			initialValues: { [testDocumentId]: testDocument, [holderDocumentId]: holderDocument }
		});
		...
		expect(result.verifiablePresentation.holder?.startsWith("did:mem")).toBeTruthy();
		expect(result.jwt.split(".").length).toEqual(3);
