...
describe("IotaIdentityProvider", () => {
	...
	test("can create a document", async () => {
		...
		console.log("DID Document", `${process.env.TEST_EXPLORER_SEARCH}${doc.id.slice(15)}`);
		...
		const assertionMethod = (doc.assertionMethod as IDidDocumentVerificationMethod[])?.find(
			v => v.publicKeyJwk?.x === Converter.bytesToBase64Url(TEST_WALLET_KEY_PAIR.publicKey)
		);
		expect(assertionMethod).toBeDefined();
		expect(assertionMethod?.id).toBe(`${doc.id}#${assertionMethod?.publicKeyJwk?.kid}`);
	...
	test("can create a verifiable credential", async () => {
		...
		expect(result.verifiableCredential.credentialSubject.id.startsWith("did:iota")).toBeTruthy();
		expect(result.verifiableCredential.credentialSubject.degreeName).toEqual(
			"Bachelor of Science and Arts"
		);
		expect(result.verifiableCredential.credentialSubject.name).toEqual("Alice");
	...
	test("can create a verifiable presentation", async () => {
		...
		expect(result.verifiablePresentation.type).toEqual("VerifiablePresentation");
